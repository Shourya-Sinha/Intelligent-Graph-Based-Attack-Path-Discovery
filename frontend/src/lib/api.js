...
export const autoConfig = () => api.get('/api/auto/config').then(r=>r.data)
export const autoConfigSet = (payload) => api.post('/api/auto/config', payload).then(r=>r.data)
export const autoEnableContinuous = (targets, interval) => api.post('/api/auto/enable-continuous', targets, {params:{interval_minutes:interval}}).then(r=>r.data)
export const autoDisable = () => api.post('/api/auto/disable').then(r=>r.data)
export const notifications = (limit=50) => api.get(`/api/notifications?limit=${limit}`).then(r=>r.data)
export const notifConfig = () => api.get('/api/notifications/config').then(r=>r.data)
export const notifConfigSet = (payload) => api.post('/api/notifications/config', payload).then(r=>r.data)
export const notifTest = () => api.post('/api/notifications/test').then(r=>r.data)
export const notifRead = (id) => api.post(`/api/notifications/read/${id}`).then(r=>r.data)
export const notifReadAll = () => api.post('/api/notifications/read-all').then(r=>r.data)
export const fixList = (jobId) => api.get(`/api/fix/${jobId}`).then(r=>r.data)
export const fixApply = (jobId, fixId) => api.post(`/api/fix/${jobId}/apply`, null, {params:{fix_id:fixId}}).then(r=>r.data)
export const fixAutonomous = (jobId) => api.post(`/api/fix/${jobId}/autonomous`).then(r=>r.data)
